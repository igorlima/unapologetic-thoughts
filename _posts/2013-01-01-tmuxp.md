...
- <details markdown="block"><summary>unapologetic things</summary>
  ...
  - [raw yaml file]({{site.baseurl}}{% link snippets/bash/tmuxp/tmuxp.unapologetic-world.yaml %})
    ...
    fetch('{{site.baseurl}}{% link snippets/bash/tmuxp/tmuxp.unapologetic-world.yaml %}')
      ...
      .then(data => {
        document.getElementById('code-block.tmuxp.unapologetic-world.yaml').innerHTML = `
          <div class="highlight">
            <pre class="highlight">
              <code>cat << EOF > .tmuxp.unapologetic-world.yaml\n${data}\nEOF</code>
            </pre>
          </div>
        `;
      });
...
- <details markdown="block"><summary>git-remote-s3</summary>
  ...
  - [raw yaml file]({{site.baseurl}}{% link snippets/bash/tmuxp/tmuxp.git-remote-s3.yaml %})
  ...
  <div id="code-block.tmuxp.git-remote-s3.yaml" class="language-bash highlighter-rouge"></div>
  <script>
    fetch('{{site.baseurl}}{% link snippets/bash/tmuxp/tmuxp.git-remote-s3.yaml %}')
      .then(response => response.text())
      .then(data => {
        document.getElementById('code-block.tmuxp.git-remote-s3.yaml').innerHTML = `
          <div class="highlight">
            <pre class="highlight">
              <code>cat << EOF > .tmuxp.git-remote-s3.yaml\n${data}\nEOF</code>
            </pre>
          </div>
        `;
      });
